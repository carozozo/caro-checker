((g) => {
  ...
  const setBasicMethodToSchema = (param) => {
    ...
    // 註冊一個輸入值轉換函式
    schema.convert = (fn) => {
      scope._pre('convert', async () => {
        scope.input = await fn(scope.helper)
      })
      return schema
    }
  ...
  const convertCheck = (param) => {
    ...
    if (ifArray(val)) return caroChecker.array(...val).required()
  ...
  const inspect = async (param) => {
    const {scope, check, val, field = undefined, insertRootHelper = false} = param
    ...
    if (!caroChecker.isSchema(checkVal)) {
      ...
      return `${field ? field : '{#label}'} only accepts ${JSON.stringify(checkVal)}`
    }
  ...
  function NumberSchema (param) {
    ...
    // 預設檢查
    scope._post(name, () => {
      if (ifNumber(scope.input)) return
      scope._setErr()
    })
  }
  ...
  function BooleanSchema (param) {
    ...
    // 預設檢查
    scope._post(name, () => {
      if (ifBoolean(scope.input)) return
      scope._setErr()
    })
  }
    ...
    scope._post(name, async () => {
      ...
      for (let i in scope.input) {
        ...
        for (let check of enums) {
          const err = await inspect({check, scope, val, field: i})
          if (err) continue
