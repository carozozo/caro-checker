  ...
  describe(`動態欄位型態檢查`, async () => {
    const plan = {
      ...
      value: cc.any().insertPre(async (helper) => {
        ...
        else if (type === 'number') schema = cc.number()

        await helper.parseSchema(schema.required())
      }),
    }

    it(`通過: {type: 'string', value: ''}`, async () => {
      const input = {type: 'string', value: ''}
      const info = await cc.object(plan).parse(input)
      assert.strictEqual(info.error, undefined)
    })
    it(`通過: {type: 'number', value: ''}`, async () => {
    ...
    it(`不通過: {type: 'number', value: ''}`, async () => {
      const input = {type: 'number', value: ''}
      const info = await cc.object(plan).parse(input)
