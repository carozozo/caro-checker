describe(__filename, () => {
  describe(`any([...enums]) - 檢查輸入值為任何格式`, async () => {
    ...
    describe(`any(cc.string(), cc.ref('elem'), undefined)`, async () => {
      const errMsg = `val only accepts String or 1 or undefined`

      const plan = {
        elem: 1,
        val: cc.any(cc.string(), cc.ref('elem'), undefined),
      }

      it(`通過: {elem: 1, val: 'str'}`, async () => {
        const input = {elem: 1, val: 'str'}
        const info = await cc.object(plan).parse(input)
        assert.strictEqual(info.error, undefined)
      })
      it(`通過: {elem: 1, val: 1}`, async () => {
        const input = {elem: 1, val: 1}
        const info = await cc.object(plan).parse(input)
        assert.strictEqual(info.error, undefined)
      })
      it(`通過: {elem: 1, val: undefined}`, async () => {
        const input = {elem: 1, val: undefined}
        const info = await cc.object(plan).parse(input)
        assert.strictEqual(info.error, undefined)
      })
      it(`不通過: {elem: 1, val: 2}`, async () => {
        const input = {elem: 1, val: 2}
        const info = await cc.object(plan).parse(input)
  ...
  describe(`any().label(label).getLabel() - 設置並取得標籤`, async () => {
    const val = `label`
    const errMsg = `${val} only accepts Any`

    it(`any().label('label').getLabel()`, async () => {
      const label = cc.any().label(val).getLabel()
      assert.strictEqual(label, val)
    })
    it(`any().label('label').insertPre((helper) => helper.markError()).parse()`, async () => {
      const info = await cc.any().label(val).insertPre((helper) => {
        helper.markError()
  ...
  describe(`any().warning(message).getWarning() - 設置並取得警告`, async () => {
    const val = `warning`
  ...
  describe(`any().note(note).getNote() - 設置並取得註解`, async () => {
    const val = `note`
  ...
  describe(`any().required([required = true]).getRequired() - 設置並取得是否必填`, async () => {
  ...
  describe(`any().skip([skip = true]).getSkip() - 設置並取得是否跳過 parse 檢查`, async () => {
